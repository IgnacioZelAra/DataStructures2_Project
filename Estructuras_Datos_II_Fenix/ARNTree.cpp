...
void ARNTree::print(NodoARN* node)
{
		if (getRoot() == NULL) {
			...
			cout << "\n Alcalde: " << node->getCanton()->getNombreAlcalde();
			cout << "\n Poblacion: " << node->getCanton()->getCantidadHabitantes();
			cout << "\n Distrito Cabecera: " << node->getCanton()->getNombreDistritoCabecera();
			cout << endl;
			if (node->getLeft()) {
				cout << "\n";
				print(node->getLeft());
...
///Parte 5
void ARNTree::showDetailsCarton(ARNTree& arnTree, NodoARN* node)
{
    if (node != nullptr)
    {
...
void ARNTree::showInverseDetailsCanton(ARNTree& arnTree, NodoARN* node){
    if (node != nullptr)
    {
